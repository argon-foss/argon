...
// Toast interface copied from FileManager
interface Toast {
  id: string;
  message: string;
  type: 'success' | 'error';
...
// Toast component copied from the FileManager
...
const ProjectSwitcher: React.FC<ProjectSwitcherProps> = ({ showOrganization = true }) => {
  ...
  const handleSwitchProject = (projectId: string, projectName: string) => {
  ...
  return (
    <>
      {/* Toast Messages - using createPortal to render directly to document.body */}
              ...
              <div className="max-h-64 overflow-y-auto">
                {projects.map((project) => (
                  <div
                    ...
                    onClick={() => {
                      handleSwitchProject(project.id, project.name);
                    }}
                  >
